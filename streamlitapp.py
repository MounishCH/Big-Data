...
def main():
    ...
    # Add this style tag to modify the font size
    st.markdown("""
        <style>
        body {
    ...
    # st. set_page_config(layout="wide")
    page = st.sidebar.radio("Navigation Pane:", ["Team", "Product Recommendation using Market Basket Analysis", "Customer Segmentation based on RFM Analysis", "Dashboard"])
    ...
    st.sidebar.markdown("### Batch B Group 09")

    # Read clean dataset
    retail = pd.read_csv('notebooks/CleanRetailData.csv', dtype={'InvoiceNo': str})
...
# Team Page
    if page == "Team":
        html_temp_team = """
        <div style="background-color:#154360;padding:6px">
        <h2 style="color:white;text-align:center;">Meet the Team - Batch B Group 09</h2>
        </div>
    """
        st.markdown(html_temp_team, unsafe_allow_html=True)
        st.markdown("")

        team = [
            {
            "name": "Mounish",
            "roll": "CB.EN.U4AIE22133",
            "image": "Mounish.jpg"
            },
            {
            "name": "Akhila",
            "roll": "CB.EN.U4AIE22104",
            "image": "akhila.jpeg"
            },
            {
            "name": "Sandeep",
            "roll": "CB.EN.U4AIE22155",
            "image": "sandeep.jpg"
            },
            {
            "name": "Pavan",
            "roll": "CB.EN.U4AIE22162",
            "image": "Pavan.jpg"
            }
            ]
        cols = st.columns(4)
        for i, member in enumerate(team):
            with cols[i]:
                ...
                st.caption(f"Roll No: {member['roll']}")
...
# %%
    ...
    if page == "Product Recommendation using Market Basket Analysis":
        ...
        option = st.selectbox('Select a Country:', country_list)
        ...
        prod_option = st.selectbox('Select a Product:', product_catalog)

        # Opening JSON file
        with open('notebooks/item_sets.json') as json_file:
    ...
    if page == "Customer Segmentation based on RFM Analysis":
        ...
        try:
            ...
            with col3:
                # Pick country
                st.markdown('## Choose Country:')
                rfm_country = st.selectbox('Select a Country for RFM Analysis:', country_list)
                ...
                rfm_country_df.loc[:, 'InvoiceDate'] = pd.to_datetime(rfm_country_df['InvoiceDate'])
...
# %%
    if page == "Dashboard":
        ...
        # Pick country
        st.markdown('#### Choose Country:')
        option = st.selectbox('Select a Country for Dashboard:', country_list)
        country_retail = choose_country(option)
